...
public class CmdCalibrateUtil {
    ...
    private static byte[] doCalibrate(byte[] src, byte[] target1, byte[] replacement1, byte[] target2, byte[] replacement2) {
        if (!BytesUtil.contains(src, target1) && !BytesUtil.contains(src, target2)) {
            return src;
        } else {
            //第一次校准
    ...
    private static byte[] algorithm(byte[] src, byte[] target, byte[] replacement) {
        ...
        byte[] temp;
        while (true) {
            index = BytesUtil.indexOf(src, target);
            if (index == -1) {
                save = BytesUtil.mergeBytes(save, src);
                break;
            }
            temp = BytesUtil.subBytes(src, 0, index);
            save = BytesUtil.mergeBytes(save, temp, replacement);
