...
public class PackageSplitUtil {
    ...
    public static PackageSplitterDto doSplit(byte[] source, int from) {
        ...
        int startIndex = BytesUtil.indexOf(source, FFFF, from);
        if (startIndex == -1) {
            return null;
        }
        int endIndex = BytesUtil.indexOf(source, FF, startIndex + FFFF.length);
        if (endIndex == -1) {
            return null;
        }
        ...
        back.setOut(BytesUtil.subBytes(source, startIndex, offset));
