...
public final class RSAUtils {
    ...
    private static final int MAX_ENCRYPT_BLOCK = 235;//前后端要一致
    ...
    public static byte[] encrypt(byte[] plainSecretKey, RSAPublicKey publicKey) throws Exception {
        ...
        int inputLen = plainSecretKey.length;
        // 保存加密的数据
        ...
        while (inputLen - offSet > 0) {
            if (inputLen - offSet > MAX_ENCRYPT_BLOCK) {
                cache = cipher.doFinal(plainSecretKey, offSet, MAX_ENCRYPT_BLOCK);
            } else {
                cache = cipher.doFinal(plainSecretKey, offSet, inputLen - offSet);
            }
            // 将加密以后的数据保存到内存
    ...
    public static String encryptToBase64(byte[] plainSecretKey, RSAPublicKey publicKey) {
        try {
            String result = Base64Utils.encodeToString(encrypt(plainSecretKey, publicKey));
