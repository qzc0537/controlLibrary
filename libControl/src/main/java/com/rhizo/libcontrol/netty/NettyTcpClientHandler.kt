 ...
 * 客户端连接回调
 */
class NettyTcpClientHandler(private var mClientListener: NettyTcpClientListener?) :
    SimpleChannelInboundHandler<String>() {
    private val TAG = "NettyTcpClientHandler"
    ...
    override fun channelRead0(ctx: ChannelHandlerContext?, frameHex: String?) {
        ...
        if (frameHex.length < 38) {
            Log.e(TAG, "云端无效指令 length:${frameHex.length}")
            return
        }
